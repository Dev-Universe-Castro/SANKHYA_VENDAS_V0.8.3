...
export async function obterToken(forceRefresh = false, retryCount = 0): Promise<string> {
  ...
  tokenPromise = (async () => {
    ...
    cachedToken = token;
      ...
      // Definir um tempo de expiração um pouco menor que o real (ex: 50 minutos para 1 hora)
      // para garantir que tenhamos tempo de renovar antes de expirar.
      const expiresAt = Date.now() + (50 * 60 * 1000);
      await redisCacheService.set('sankhya:token', { token, expiresAt, geradoEm: new Date().toISOString() } as TokenCache, 50 * 60 * 1000); // 50 minutos

      // Atualizar cache do painel admin (se disponível)
      try {
        const { setTokenCache } = await import('@/app/api/admin/token-info/route');
        setTokenCache(token);
      } catch (e) {
